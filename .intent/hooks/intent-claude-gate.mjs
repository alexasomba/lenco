...
const AGENT = 'claude'
const CATALOG_COMMAND =
  'pnpm dlx @tanstack/intent@latest list --json --no-notices'
...
async function main() {
  ...
  if (isSessionStartEvent(event)) {
    ...
    const additionalContext = await createSessionCatalogContext(
      rootForEvent(event),
    )
    appendObservation(stateFile, { action: 'list', raw: CATALOG_COMMAND })
    if (additionalContext) {
      process.stdout.write(
        JSON.stringify(sessionStartOutput(additionalContext)),
      )
    }
    return
  }
...
function formatSkillCatalog(skills) {
  return skills
    .map(
      (skill) => `- ${skill.use}: ${normalizeDescription(skill.description)}`,
    )
    .join('\n')
}
